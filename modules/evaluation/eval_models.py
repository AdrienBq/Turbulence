...
'''
# Description:
Run this file to have evaluations and plots of the different models.
See each model's description in the folder model_train for more details.

# modify the evaluation file :
- change the model names on line 482
- define another model, add it to the list of model names, add an "elif" statement for this model
- comment / uncomment ploting statements or print statements
'''
...
def main():
    ...
    model_names = ["conv", "pca", "simple", "vae", 'ae', 'conv_ae', 'conv_ae_d', 'multiL', 'multiL_d', 'meta_d', '163264', '1632', '3264']
    ...
    for i in range(len(model_names)) :
        ...
        if name == "conv" :
            ...
            model.load_state_dict(torch.load('models/{}_net.pt'.format(name), map_location=torch.device('cpu')))
            ...
            for var in variables[:-1] :
                ...
                ae_model.load_state_dict(torch.load('models/{}_ae_net.pt'.format(var), map_location=torch.device(device)))
                ae_model.to(device)
                ae_model.eval()
                ae_models.append(ae_model)
            model = AE_DNN(ae_models[0], ae_models[1], ae_models[2], ae_models[3], ae_models[4], ae_models[5], nn_model)
            model.load_state_dict(torch.load('models/dnn_ae_net.pt', map_location=torch.device('cpu')))
    ...
    # plot : uncomment the lines below and change the name of the path to save the plot
    '''for i in range(len(model_names)) :
    ...
    # uncomment the line bellow to plot the baseline model
