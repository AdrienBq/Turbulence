...
'''
## Description
This is a test where we trained a model that uses one auto-encoder for each input variable.
'''
...
class AE(nn.Module):
    '''
    ## Description
    AE neural network for the inputs. Change the variable on line 225.
    ...
    def __init__(self, input_features=376,  hidden_size1=256, hidden_size2=128, z_dim=3, 
        ...
        - drop_enc3: dropout probability in the encoder third hidden layer, default=0.3
...
def test(model, var, device, input_test):
    '''
    ## Description
    ...
    x_reconst = model(input_batch)
    # compute loss
    reconst_loss = F.mse_loss(x_reconst, input_batch, reduction='mean')
    test_loss += reconst_loss.item()
    return test_loss

def train(device, var, lr_ae, decay_ae, batch_size, nb_epochs, train_losses, test_losses, input_train, input_test, len_in):
    '''
    ## Description
    ...
    optimizer_name = "Adam"
    optimizer_vae = getattr(optim, optimizer_name)(model_ae.parameters(), lr=lr_ae)

    optimizer_vae = torch.optim.Adam(model_ae.parameters(), lr=lr_ae)
    scheduler_vae = torch.optim.lr_scheduler.ExponentialLR(optimizer_vae, decay_ae, last_epoch= -1)
    kl_factor = 0
    kl_start_epoch = 20
    ...
    for epoch in trange(nb_epochs, leave=False):
        ...
        test_losses.append(test(model_ae, var, device, input_test))
...
def main():
    ...
    var = 5     # 0 = u, 1 = v, 2 = w, 3 = theta, 4 = s, 5 = tke, 6 = wtheta
    lr_vae = 0.00094
    decay_vae = 0.99
    ...
    torch.save(model.state_dict(), f"models/{variables[var]}_ae_net.pt")
